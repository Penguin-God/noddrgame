...
public class MoveOther : MonoBehaviour
{
    ...
    public void Move(string dir)
    {
        ...
        {
            StartCoroutine(NpcOtherMove(MoveObject));
            NotCoroutine = true;
        }
    }

    IEnumerator NpcOtherMove(GameObject moveObject)
    {
        while (DirSave.Count != 0) // DirSave의 값이 모두 제거되면 멈춤
        {
            string direction = DirSave.Dequeue(); //queue에서 제거되는 값이 switch문의 값이됨
            objectVector.Set(0, 0); //코루틴을 한번 돌고나면 백터값을 초기화(x,y가 동시에 1을가지면 안되기 때문)
            ...
            {
                moveObject.transform.Translate(objectVector.x * speed, objectVector.y * speed, 0);
    ...
    void OtherAinmation()
    {
        ...
        animator.SetFloat("DirX", objectVector.x);
